...
import { Plus, Search, Trash2, Edit3, X, Upload, ArrowLeft, Save, CheckCircle2, FolderPlus, RotateCcw, Clock } from "lucide-react";
...
export const AdminBlogManager: React.FC<AdminBlogManagerProps> = ({
    ...
    if (saved) {
      try {
        ...
        if (parsed?.timestamp && (parsed.data?.title || parsed.data?.content || parsed.data?.excerpt)) {
          const dateStr = new Date(parsed.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  ...
  // Autosave to localStorage on every change in form mode
  useEffect(() => {
    if (viewMode === "form") {
  ...
  const loadData = async () => {
    try {
      ...
      if (blogRes.ok) {
        const postsData = await blogRes.json();
        setPosts(postsData);
        if (targetId) {
          const target = postsData.find((p: any) => p._id === targetId);
      ...
      if (catRes.ok) {
        ...
        if (!formData.category && catData.length > 0) {
          setFormData((prev: any) => ({ ...prev, category: catData[0].name }));
        }
      }
    } catch (err) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setFormData((prev: any) => ({
        ...
        imageAlt: prev.imageAlt || prev.title || "Blog cover image"
      }));
  ...
  const executeCreateCategory = async (nameToCreate: string) => {
    ...
    try {
      ...
      const createdCat = data.name ? data : { name: trimmed, slug, type: "blog" };
      ...
      setCategories((prev) => {
        const exists = prev.some((c) => (c.name || "").toLowerCase() === trimmed.toLowerCase());
        if (exists) return prev;
        return [...prev, { name: trimmed, slug: trimmed.toLowerCase(), type: "blog" }];
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Immediate optimistic update in post list
      setPosts((prev) => {
      ...
      setMessage({ type: "success", text: editingId ? "✓ Blog updated successfully!" : "✓ Blog post published immediately!" });
      setEditingId(null);
      setDraftRecovered(null);
      triggerBackToList();
    } catch (err: any) {
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {viewMode === "list" && (
        <div className="space-y-4">
          ...
          {loading ? (
            <div className="py-20 text-center font-mono text-sm text-[#6B6862]">Loading blog posts...</div>
      ...
      {viewMode === "form" && (
        <form onSubmit={handleSubmit} className="bg-[#FAF8F2] border-2 border-[#141413] rounded-3xl p-6 sm:p-10 shadow-[6px_6px_0px_#141413] space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              <label className="block font-mono text-xs text-[#6B6862] uppercase mb-2">Full Article Content (Quill Editor) *</label>
              <RichTextEditor
